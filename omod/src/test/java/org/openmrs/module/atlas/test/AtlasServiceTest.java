...
public class AtlasServiceTest extends BaseModuleContextSensitiveTest {
	...
	public void runBeforeEachTest() throws Exception {
		atlasSrv = new AtlasServiceImpl();// Context.getService(AtlasService.class);
...
//	
	 ...
	 */
	@Test
	public void getAtlasData_shouldInitializeWithDefaultValuesSeeConstructorInAtlasDatajavaAllAtlasDataFieldsExceptIdThatDoNotHaveCorrespondingGlobalProperties() throws Exception {
	 ...
	 */
	@Test
	public void getAtlasData_shouldThrowJavalangIllegalArgumentExceptionWhenAtlasidGlobalPropertyDoesNotExist() throws Exception {
...
//		Boolean isDirty = Boolean.parseBoolean(getGlobalPropertyObject(AtlasConstants.GLOBALPROPERTY_IS_DIRTY).getPropertyValue());
...
//	}
	 ...
	 */
	@Test
	public void setAtlasBubbleData_shouldOnlySetTheAtlasDataGlobalPropertiesThatAreRelatedToTheAtlasBubble() throws Exception {
		...
		assertTrue("Non bubble properties should not have changed", Double.parseDouble(getGlobalPropertyObject(AtlasConstants.GLOBALPROPERTY_LATITUDE).getPropertyValue())  == latitude);
		assertTrue("Non bubble properties should not have changed", Double.parseDouble(getGlobalPropertyObject(AtlasConstants.GLOBALPROPERTY_LONGITUDE).getPropertyValue()) == longitude);
		assertTrue("Non bubble properties should not have changed", Integer.parseInt(getGlobalPropertyObject(AtlasConstants.GLOBALPROPERTY_ZOOM).getPropertyValue()) == zoom);
		assertTrue("Non bubble properties should not have changed", Boolean.parseBoolean(getGlobalPropertyObject(AtlasConstants.GLOBALPROPERTY_USAGE_DISCLAIMER_ACCEPTED).getPropertyValue())== usageDisclamerAccepted);
		assertTrue("Non bubble properties should not have changed", Boolean.parseBoolean(getGlobalPropertyObject(AtlasConstants.GLOBALPROPERTY_MODULE_ENABLED).getPropertyValue()) == moduleEnabled);
		assertTrue("Non bubble properties should not have changed", Boolean.parseBoolean(getGlobalPropertyObject(AtlasConstants.GLOBALPROPERTY_INCLUDE_SYSTEM_CONFIGURATION).getPropertyValue()) == includeSystemConfiguration);
	...
	public void setIncludeSystemConfiguration_shouldNotModifyAtlasisDirtyGlobalProperty() throws Exception {
		...
		assertTrue("IsDirty should remained unchanged (should be false).", 
...
//	}
	...
	private void setGlobalProperty(String name, String value) {
		...
		try {
			...
			System.out.println("Could not set global property: (" + name +" - "+value+"). Exception:" + apiEx.getMessage());
		}
	}
